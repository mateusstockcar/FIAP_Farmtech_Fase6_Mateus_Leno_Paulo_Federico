# 🌾 FarmTech Solutions - Projeto Fase 6: Redes Neurais e Visão Computacional

Este repositório contém a solução completa para a Fase 6 (IA) da FIAP, focada em sistemas de visão computacional para segurança e monitorização animal/patrimonial.

## 👥 Integrantes (Grupo S - Turma 31)
* Federico Enrique Villagra - RM 567187
* Paulo Henrique Benfica - RM 567648
* Mateus Penteado de Lima - RM 568518
* Leno Siqueira - RM 567893

## 📺 Vídeo de Demonstração
[ASSISTIR VÍDEO NO YOUTUBE](COLOQUE_O_LINK_AQUI)

## 📁 Conteúdo
1. **YOLOv5 Customizado:** Reconhecimento de classes específicas com comparação de 30 e 60 épocas.
2. **Comparação de Abordagens:** YOLO Custom vs YOLO Tradicional vs CNN do zero.
3. **Análise de Performance:** Avaliação de acurácia, precisão e tempo de inferência.
